...
export class LoginComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    style.type = "text/css";
    style.innerHTML =
      ".backg { background-image: url('../../assets/blurred.jpg'); }";
  ...
  onSubmit() {
    this.userService
      .getUser(this.login.UserName, this.login.Password)
      .subscribe(
        success => {
        ...
        err => {
          console.log(
            `Usuario inautorizado, error: ${
              err.status
            }, cuerpo del error: ${JSON.stringify(err.error)}`
          );
