...
export class ShowDetailsComponent implements OnInit {
  ...
  makeExpectativesRequest() {
    this.expectativeService.getExpectativeByPersonID(this.p._id).subscribe(
      success => {
        this.e = success[0];
      },
      (err: HttpErrorResponse) => {
        if (err.error instanceof Error) {
          ...
          this.openSnackBar(
            `Error al solicitar el documento de expectativas de la organización`,
            "Ok",
            "red-snackbar"
          );
        }
  ...
  makeEducationRequest() {
    this.educationService.getEducationByPersonID(this.p._id).subscribe(
      success => {
        this.ed = success[0];
      },
      (err: HttpErrorResponse) => {
        if (err.error instanceof Error) {
          ...
          this.openSnackBar(
            `Error al solicitar el documento de educación`,
            "Ok",
            "red-snackbar"
          );
        }
  ...
  makeSocioEconomicRequest() {
    this.socioEconomicService.getSocioEconomicByPersonID(this.p._id).subscribe(
      success => {
        this.s = success[0];
      },
      (err: HttpErrorResponse) => {
        if (err.error instanceof Error) {
          ...
          this.openSnackBar(
            `Error al solicitar el documento de aspectos socioeconómicos`,
            "Ok",
            "red-snackbar"
          );
        }
  ...
  makeEmploynmentRequest() {
    this.employnmentService.getEmploynmentPersonID(this.p._id).subscribe(
      success => {
        this.emp = success[0];
      },
      (err: HttpErrorResponse) => {
        if (err.error instanceof Error) {
          ...
          this.openSnackBar(
            `Error al solicitar el documento de emppleo`,
            "Ok",
            "red-snackbar"
          );
        }
  ...
  makeHealthRequest() {
    this.healthService.getHealthByPersonID(this.p._id).subscribe(
      success => {
        this.h = success[0];
        this.loading = false;
      },
      (err: HttpErrorResponse) => {
        if (err.error instanceof Error) {
          ...
          this.openSnackBar(
            `Error al solicitar el documento de salud`,
            "Ok",
            "red-snackbar"
          );
        }
