...
export class AddEducationComponent implements OnInit {
  ...
  onSubmit() {
    ...
    else this.educationService.updateEducationDoc(this.education);
  }

  addCourse(course: string) {
    if (course.length !== 0) this.education.Courses.unshift(course);
    else
      this.openSnackBar(
        "Debe agregar el nombre de un curso",
        "Ok",
        "red-snackbar"
      );
  }
