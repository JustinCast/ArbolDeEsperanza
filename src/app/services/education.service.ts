...
export class EducationService {
  ...
  getEducationByPersonID(personID: string): Observable<Education> {
    return this._http.get<Education>(
      `${environment.SERVER_BASE_URL}api/education/getEducationByPersonID/${personID}`
    );
  }

  saveEducationDoc(doc: Education) {
    this._http.post(`${environment.SERVER_BASE_URL}api/education/saveEducationDoc`, doc).subscribe(
      success => {
        this.openSnackBar(
  ...
  verifyExistency(PersonID: string) {
    this._http
      .get<Array<any>>(`${environment.SERVER_BASE_URL}api/education/verifyExistency/${PersonID}`)
      .subscribe(
        success => {
          console.log(success);
  ...
  updateEducationDoc(doc: Education) {
    this._http.put(`${environment.SERVER_BASE_URL}api/education/updateEducationDoc`, doc).subscribe(
      success => {
      ...
      (err: HttpErrorResponse) => {
        if (err.error instanceof Error) {
          ...
          this.openSnackBar(
            `Error con la actualización del documento`,
            "Ok",
            "red-snackbar"
          );
        }
